...
export class PenComponent implements OnInit {
  ...
  constructor(private readonly fb: FormBuilder, private http: HttpClient,
    private penServcie: PenService,private readonly sanitizer: DomSanitizer,
    private readonly storageSrv: StorageService,
     private route: ActivatedRoute,private router:Router) { }

  ngOnInit(): void {
    this.pen_form_group = new FormGroup({
    ...
    this.route.queryParams.pipe(
      map(params => {
        ...
        if (this.userId) {
          return this.penServcie.findIdByUser(this.userId).then((data: any) => this.initializeForm(data));
        }
        this.initializeForm();
        return EMPTY;
      }),
  ...
  selectedFile($event: any) {
    ...
    if (file) {
      this.pen_form_group.get('profileImageFile')?.setValue(file);
      this.pen_form_group.get('profileImage')?.setValue(this.sanitizer.bypassSecurityTrustUrl(URL.createObjectURL(file)));
    }

  }
  get profileImage() {
    return this.pen_form_group.get('profileImage')?.value;
  }
  ...
  save(pen_form_group: FormGroup) {
    const formValue = { ...this.pen_form_group.value };
    const uuid = uuidv4();
    ...
    this.storageSrv.uploadImage(formValue.profileImageFile, uuid).pipe(
      switchMap(profileUrl => {
        ...
        return this.penServcie.saveUser(formValue);
      })).subscribe();
      pen_form_group.reset()
      this.router.navigate(['/pen'])

  }

  
  private initializeForm(initData?: Desktop) {
    this.pen_form_group = this.fb.group({
