...
import { GamingService } from 'src/app/service/gaming.service';
import { DomSanitizer } from '@angular/platform-browser';
import { ActivatedRoute, Router } from '@angular/router';
import { map, EMPTY, switchMap } from 'rxjs';
import { Gaming } from 'src/app/models/gaming.model';
import { StorageService } from 'src/app/service/storage.service';
import {  uuidv4 } from '@firebase/util';

@Component({
  selector: 'app-gaming-form',
  templateUrl: './gaming-form.component.html',
  styleUrls: ['./gaming-form.component.css']
})
export class GamingFormComponent implements OnInit {

  gaming_form_group!: FormGroup;
  ...
  constructor(private readonly fb: FormBuilder, private http: HttpClient,
    private gamingService: GamingService,private readonly sanitizer: DomSanitizer,
    private readonly storageSrv: StorageService,
     private route: ActivatedRoute,private router:Router) { }

  ngOnInit(): void {
    this.gaming_form_group = new FormGroup({
    ...
    this.route.queryParams.pipe(
      map(params => {
        ...
        if (this.userId) {
          return this.gamingService.findIdByUser(this.userId).then((data: any) => this.initializeForm(data));
        }
        this.initializeForm();
        return EMPTY;
      }),
  ...
  selectedFile($event: any) {
    ...
    if (file) {
      this.gaming_form_group.get('profileImageFile')?.setValue(file);
      this.gaming_form_group.get('profileImage')?.setValue(this.sanitizer.bypassSecurityTrustUrl(URL.createObjectURL(file)));
    }

  }
  get profileImage() {
    return this.gaming_form_group.get('profileImage')?.value;
  }
  ...
  save(gaming_form_group: FormGroup) {
    const formValue = { ...this.gaming_form_group.value };
    const uuid = uuidv4();
    ...
    this.storageSrv.uploadImage(formValue.profileImageFile, uuid).pipe(
      switchMap(profileUrl => {
        ...
        return this.gamingService.saveUser(formValue);
      })).subscribe();
      gaming_form_group.reset()
      this.router.navigate(['/gaming'])

  }

  
  private initializeForm(initData?: Gaming) {
    this.gaming_form_group = this.fb.group({
