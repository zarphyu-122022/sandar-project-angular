...
export class DesktopFormComponent implements OnInit {
  ...
  constructor(private readonly fb: FormBuilder, private http: HttpClient,
    private desktopService: DesktopService,private readonly sanitizer: DomSanitizer,
  ...
  ngOnInit(): void {
    ...
    this.route.queryParams.pipe(
      map(params => {
        ...
        if (this.userId) {
          return this.desktopService.findIdByUser(this.userId).then((data: any) => this.initializeForm(data));
        }
        this.initializeForm();
        return EMPTY;
      }),
  ...
  selectedFile($event: any) {
    ...
    if (file) {
      this.desktop_form_group.get('profileImageFile')?.setValue(file);
      this.desktop_form_group.get('profileImage')?.setValue(this.sanitizer.bypassSecurityTrustUrl(URL.createObjectURL(file)));
    }

  }
  get profileImage() {
    return this.desktop_form_group.get('profileImage')?.value;
  }
  ...
  save(desktop_form_group: FormGroup) {
    const formValue = { ...this.desktop_form_group.value };
    const uuid = uuidv4();
    ...
    this.storageSrv.uploadImage(formValue.profileImageFile, uuid).pipe(
      switchMap(profileUrl => {
        ...
        return this.desktopService.saveUser(formValue);
      })).subscribe();
  ...
  private initializeForm(initData?: Desktop) {
    this.desktop_form_group = this.fb.group({
